...
# API
The module only takes in variadics, so arrays of Vector2 values should be unpacked using ```luau table.unpack``` whenever a Curve object is created
...
Sampling a curve
```luau local vec2 = curve.sample(12) -- input is any x value within the domain of your vectors array```

You can also sample a parametric curve
```luau local vec2 = curve.sample(0.7) -- input is a value between [0, 1], which corresponds with the beginning and end of your curve```
